...
myClock = do
  ...
  where
    ...
    offset z = do
      ...
      return (x:y:z:':':xs)
    showFormat z f = return . formatTime Data.Time.Format.defaultTimeLocale f . utcToZonedTime z =<< Clock.getCurrentTime
...
main = defaultTaffybar defaultTaffybarConfig
  { startWidgets = [pager] --pad
  , endWidgets = [ myClock --pad
                 , mem
                 , cpu
                 , bat
                 , tray
